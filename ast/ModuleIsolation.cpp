...
std::string getTopLevelDeclName(const ASTNode* node) {
    ...
    case NodeType::NODE_VAR_DECL:
        return static_cast<const VarDecl*>(node)->name;
    case NodeType::NODE_FUN_DECL:
        return static_cast<const FunDecl*>(node)->name;
    case NodeType::NODE_CLASS_DECL:
        return static_cast<const ClassDecl*>(node)->name;
    default:
        return "";
    }
}
...
ModuleTopLevelRenamer::ModuleTopLevelRenamer(const std::string& modulePath) : modulePath_(modulePath) {
    // 模块顶层作用域（栈底）
...
void ModuleTopLevelRenamer::collectNonExportTopLevelNames(Block& moduleAst) {
    ...
    for (const auto& stmt : moduleAst.statements) {
        ...
        if (stmt->nodeType == NodeType::NODE_EXPORT_STMT)
            continue;
...
void ModuleTopLevelRenamer::renameInNode(ASTNode* node) {
    ...
    case NodeType::NODE_FUN_DECL: {
        ...
        for (const auto& param : n->params)
            defineInCurrentScope(param);
        if (n->body)
            renameInNode(n->body.get());
        popScope();
    ...
    case NodeType::NODE_FUN_CALL: {
        ...
        if (n->callee)
            renameInNode(n->callee.get());
        for (const auto& arg : n->arguments) {
            if (arg)
                renameInNode(arg.get());
        }
        break;
    }
    // ---- 控制流节点：递归处理子节点 ----
    case NodeType::NODE_IF_STMT: {
        ...
        if (n->condition)
            renameInNode(n->condition.get());
        if (n->thenBranch)
            renameInNode(n->thenBranch.get());
        if (n->elseBranch)
            renameInNode(n->elseBranch.get());
        break;
    }
    ...
    case NodeType::NODE_BINARY_OP: {
        ...
        if (n->left)
            renameInNode(n->left.get());
        if (n->right)
            renameInNode(n->right.get());
        break;
    }
    ...
    case NodeType::NODE_DICT_LITERAL: {
        ...
        for (const auto& p : n->pairs) {
            if (p.first)
                renameInNode(p.first.get());
            if (p.second)
                renameInNode(p.second.get());
        }
        break;
    }
    case NodeType::NODE_INDEX_ACCESS: {
        ...
        if (n->object)
            renameInNode(n->object.get());
        if (n->index)
            renameInNode(n->index.get());
        break;
    }
    ...
    case NodeType::NODE_TRY_STMT: {
        ...
        if (n->tryBlock)
            renameInNode(n->tryBlock.get());
        // catch 块进入新作用域，catchVarName 是局部变量
        pushScope();
        if (!n->catchVarName.empty())
            defineInCurrentScope(n->catchVarName);
        if (n->catchBlock)
            renameInNode(n->catchBlock.get());
        popScope();
