...
const std::vector<SandboxLevel>& SandboxLibrary::levels() {
    static const std::vector<SandboxLevel> kLevels = {
        ...
        SandboxLevel{1,
                     ...
                     "💡 步骤：点 PUSH_INT 1（栈出现 1）→ PUSH_INT 2（栈变 [1,2]）→ ADD（弹出 1 和 2，压入 3，栈变 "
                     "[3]）→ PRINT（弹出 3 并打印）。\n"
                     "   注意 ADD 永远吃「最上面两个」，顺序很重要：先压 1 再压 2，弹出来时是先 2 后 "
                     "1，但加法交换律让结果一样。",
                     1},
        ...
        SandboxLevel{2,
                     ...
                     "💡 先把三个数都压栈：[1, 2, 3]；再用 MUL 让栈顶两个(2,3)相乘得 6 → [1, 6]；最后 ADD 让 1+6=7 → "
                     "[7]；PRINT。\n"
        ...
        SandboxLevel{3,
                     ...
                     "9",
                     "💻 同数不同顺序：关卡 2 和 3 "
                     "用完全相同的「可用指令按钮」，只是点击顺序不同（括号改变了结合顺序），结果就从 7 变成 9。\n"
        ...
        SandboxLevel{5,
                     ...
                     "• SUB / DIV / MOD 都是二元运算，吃栈顶两个：先弹出的作右操作数，后弹出的作左操作数（即 a - b 中 "
                     "a 先入栈）。\n"
                     "• 栈式 VM 没有「变量名」概念，所有运算都靠栈顶那几个值——这正是它比寄存器 VM "
                     "更简单、也更「笨」的地方：人要知道数据在栈的哪一层。",
                     "💡 实验：\n"
