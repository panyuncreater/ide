...
const std::vector<StepOpCodeDocEntry>& StepExplainerLibrary::opCodeDocs() {
    static const std::vector<StepOpCodeDocEntry> kDocs = {
        ...
        {"OP_INT", "常量加载", "常量池索引(2B)", "压入 1 个值",
         "从常量池读取整数常量并压入栈顶。整数采用 int64_t 编码，NaN-boxing 标签位区分 int/float。", "var x = 42;"},
        {"OP_FLOAT", "常量加载", "常量池索引(2B)", "压入 1 个值",
         "从常量池读取浮点常量并压入栈顶。浮点采用 IEEE 754 双精度编码。", "var pi = 3.14;"},
        {"OP_STRING", "常量加载", "常量池索引(2B)", "压入 1 个值",
         "从常量池读取字符串常量并压入栈顶。字符串为引用计数堆对象，常量池去重避免重复分配。", "var s = \"hello\";"},
        {"OP_NULL", "常量加载", "无", "压入 1 个值",
         "将 null 值压入栈顶。null 在 NaN-boxing 中用专用标签表示，与 false 区分。", "var n = null;"},
        {"OP_TRUE", "常量加载", "无", "压入 1 个值", "将布尔常量 true 压入栈顶。", "var b = true;"},
        {"OP_FALSE", "常量加载", "无", "压入 1 个值", "将布尔常量 false 压入栈顶。", "var b = false;"},
        // ---- 2. 算术运算（6）----
        ...
        {"OP_SUBTRACT", "算术运算", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值（右、左），执行左减右后压入结果。整数减法截断向零，三后端统一。", "var z = x - y;"},
        {"OP_MULTIPLY", "算术运算", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值相乘后压入结果。整数乘法截断向零，溢出回绕为 int64_t 补码语义。", "var z = x * y;"},
        {"OP_DIVIDE", "算术运算", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值相除后压入结果。整数除法截断向零（三后端统一），浮点除法遵循 IEEE 754。", "var z = x / y;"},
        {"OP_MODULO", "算术运算", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值取模后压入结果。符号与被除数一致（截断向零除法的余数），三后端统一。", "var z = x % y;"},
        {"OP_NEGATE", "算术运算", "无", "弹出 1 个值，压入 1 个结果",
         "弹出栈顶值取负后压入。对整数取负即 0-x，对浮点翻转符号位。", "var z = -x;"},
        // ---- 3. 逻辑比较（9）----
        {"OP_NOT", "逻辑比较", "无", "弹出 1 个值，压入 1 个结果",
         "弹出栈顶值取逻辑非后压入布尔结果。null/false 为假，其余为真。", "var b = !x;"},
        {"OP_EQUAL", "逻辑比较", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值进行相等比较后压入布尔结果。跨类型比较（int vs float）按值比较。", "var b = x == y;"},
        {"OP_NOT_EQUAL", "逻辑比较", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值进行不等比较后压入布尔结果。等价于 OP_EQUAL 结果取反。", "var b = x != y;"},
        {"OP_LESS", "逻辑比较", "无", "弹出 2 个值，压入 1 个结果",
         "弹出栈顶两个值（右、左），若左 < 右则压入 true。仅对数字类型有效。", "var b = x < y;"},
        ...
        {"OP_OR", "逻辑比较", "无", "无效果（已废弃）",
         ...
         "var b = x or y;"},
        // ---- 4. 栈操作（4）----
        {"OP_POP", "栈操作", "无", "弹出 1 个值", "弹出栈顶值并丢弃。表达式语句副作用求值后丢弃返回值，防止栈泄漏。",
         "foo();"},
        {"OP_DUP", "栈操作", "无", "压入 1 个值（复制栈顶）",
         "复制栈顶值并压入。用于需要保留操作数副本的场景（如复合赋值的中间步骤）。", "x = x + 1;"},
        {"OP_DUP_N", "栈操作", "深度(1B)", "压入 1 个值",
         "复制栈中第 N 个值到栈顶（不清除原值）。用于写回时保留索引值。", "arr[i] = v;"},
        ...
        {"OP_DEFINE_VAR", "变量访问", "名称索引(2B)", "弹出 1 个值",
         "弹出栈顶值，以常量池中名称为键首次写入全局表。与 OP_SET_GLOBAL 的已存在赋值语义区分。", "var x = 10;"},
        {"OP_GET_VAR", "变量访问", "名称索引(2B)", "压入 1 个值", "按名称读取全局变量值并压栈。名称在常量池中索引。",
         "print(x);"},
        {"OP_SET_VAR", "变量访问", "名称索引(2B)", "弹出 1 个值",
         "弹出栈顶值，以常量池中名称为键写入全局表（已存在变量的赋值）。", "x = 10;"},
        {"OP_DELETE_VAR", "变量访问", "名称索引(2B)", "无效果",
         "删除全局变量。用于块作用域退出时清理全局变量（如 for 循环变量）。", "// 块作用域退出自动触发"},
        {"OP_GET_LOCAL", "变量访问", "槽位(1B)", "压入 1 个值",
         "读取当前栈帧中局部变量槽位并压栈。槽位是编译期分配的栈相对位置，访问 O(1)。", "// 函数内访问局部变量"},
        {"OP_SET_LOCAL", "变量访问", "槽位(1B)", "弹出 1 个值", "弹出栈顶值写入当前栈帧的局部变量槽位。", "x = 10;"},
        {"OP_GET_GLOBAL", "变量访问", "全局槽位(2B)", "压入 1 个值",
         "读取全局槽位的值并压栈。槽位在编译期由 GlobalSlotAllocator 分配，运行时 vector 直接访问。", "print(x);"},
        {"OP_SET_GLOBAL", "变量访问", "全局槽位(2B)", "弹出 1 个值", "弹出栈顶值写入全局槽位。", "x = 10;"},
        {"OP_DEFINE_GLOBAL", "变量访问", "全局槽位(2B)", "弹出 1 个值",
         "弹出栈顶值，首次写入全局槽位（定义新全局变量）。", "var x = 10;"},
        {"OP_DELETE_GLOBAL", "变量访问", "全局槽位(2B)", "无效果", "删除全局槽位变量。用于块作用域退出时清理。",
         "// 块作用域退出自动触发"},
        // ---- 6. 控制流（6）----
        {"OP_JUMP", "控制流", "跳转目标(2B)", "无效果",
         "无条件跳转到指定绝对字节偏移。用于 if/else 分支跳过 false 块。", "if (true) { print(\"yes\"); }"},
        {"OP_JUMP_IF_FALSE", "控制流", "跳转目标(2B)", "无效果（不消费条件值）",
         "检查栈顶条件，若为 false 则跳转到指定偏移。不弹出条件值，由编译器额外生成 OP_POP。", "if (cond) { ... }"},
        {"OP_LOOP", "控制流", "跳转目标(2B)", "无效果",
         "回跳到循环入口（负偏移，实际编码为绝对偏移）。用于 while/for 循环回边。", "while (cond) { ... }"},
        ...
        {"OP_GET_UPVALUE", "函数调用", "upvalue索引(1B)", "压入 1 个值",
         "读取闭包捕获的外层变量。若 upvalue 仍开放则从栈帧读取，已关闭则从堆读取。", "// 闭包内访问外层变量"},
        ...
        {"OP_BUILD_ARRAY", "容器操作", "元素个数(1B)", "弹出 N 个值，压入 1 个结果",
         "弹出栈顶 N 个元素构建 ArrayData 并压入。数组为引用计数堆对象，COW 写时拷贝。", "var arr = [1, 2, 3];"},
        ...
        {"OP_BUILD_TUPLE", "容器操作", "元素个数(1B)", "弹出 N 个值，压入 1 个结果",
         "弹出栈顶 N 个元素构建 TupleData 并压入。元组不可变（immutable），用于多返回值与解构。", "var t = (1, 2, 3);"},
        {"OP_INDEX_GET", "容器操作", "无", "弹出 2 个值，压入 1 个结果",
         "弹出索引与容器，读取 container[index] 并压栈。支持数组/字典/字符串/元组。", "var v = arr[0];"},
        {"OP_INDEX_SET", "容器操作", "无", "弹出 3 个值",
         "弹出值、索引、容器，执行 container[index] = value。COW 写时拷贝确保独占所有权。", "arr[0] = 99;"},
        {"OP_INDEX_SET_VAR", "容器操作", "名称索引(2B)", "弹出 2 个值",
         "弹出值与索引，直接修改 globals_[varName][index] = value。优化路径避免重复 load/store。", "arr[i] = 99;"},
        {"OP_INDEX_SET_LOCAL", "容器操作", "槽位(1B)", "弹出 2 个值",
         "弹出值与索引，直接修改 stack_[bp+slot][index] = value。局部变量数组赋值的优化路径。", "arr[i] = 99;"},
        ...
        {"OP_MEMBER_GET", "成员与类", "名称索引(2B)", "弹出 1 个值，压入 1 个结果",
         "弹出实例，读取字段 name 的值并压栈。通过 fieldSlotIndex 查表定位字段槽位。", "var v = p.x;"},
        {"OP_MEMBER_SET", "成员与类", "名称索引(2B)", "弹出 2 个值", "弹出值与实例，写入字段 name。实例字段槽位赋值。",
         "p.x = 10;"},
        {"OP_MEMBER_SET_VAR", "成员与类", "变量索引(2B) + 字段索引(2B)", "弹出 1 个值",
         "弹出值，直接修改 globals_[varName].fields[fieldIdx] = value。全局变量成员赋值的优化路径。", "p.x = 10;"},
        {"OP_MEMBER_SET_LOCAL", "成员与类", "槽位(1B) + 字段索引(2B)", "弹出 1 个值",
         "弹出值，直接修改 stack_[bp+slot].fields[fieldIdx] = value。局部变量成员赋值的优化路径。", "p.x = 10;"},
        ...
        {"OP_CLASS_NEW", "成员与类", "名称索引(2B) + 参数个数(1B)", "弹出 N+1 个值，压入 1 个结果",
         "类构造：弹出 N 个参数 + 类模板，创建 InstanceData 并调用 init 方法初始化字段。", "var p = Point(3, 4);"},
        ...
        {"OP_DEFINE_CLASS", "成员与类", "名称索引(2B) + 类信息(2B)", "弹出 1 个值",
         "从栈顶 pop 模板实例，提取类信息，注册到 VM 类表。类声明编译期的最终步骤。", "class Point { ... }"},
        {"OP_SUPER_CALL", "成员与类", "名称索引(2B) + 参数个数(1B) + 接收者(2B+1B) + 类索引(2B)",
         "弹出 N+1 个值，压入 1 个结果",
         "调用父类方法：从当前实例的父类链查找方法并调用。super 语义三后端统一，"
         "确保继承链中的方法回退正确。",
         "super.foo();"},
        {"OP_SUPER_MEMBER_GET", "成员与类", "名称索引(2B)", "弹出 1 个值，压入 1 个结果",
         "从父类查找字段并读取。super 成员访问语义三后端统一。", "super.x;"},
        ...
        {"OP_PRINT", "其他", "无", "弹出 1 个值",
         "弹出栈顶值并输出到标准输出（带换行）。Value::toString 序列化所有类型。", "print(x);"},
        {"OP_WRITEBACK_MEMBER_VAR", "其他", "变量索引(2B) + 字段索引(2B)", "弹出 1 个值",
         "成员写回到全局变量：从 lastMutatedReceiver_ 取基对象，写入字段。用于嵌套左值变异 a.b.c = ...", "a.b.c = 1;"},
        {"OP_WRITEBACK_MEMBER_LOCAL", "其他", "槽位(1B) + 字段索引(2B)", "弹出 1 个值",
         "成员写回到局部变量：从 lastMutatedReceiver_ 取基对象，写入字段。", "a.b.c = 1;"},
        {"OP_WRITEBACK_INDEX_VAR", "其他", "变量索引(2B)", "弹出 1 个值",
         "索引写回到全局变量：索引从栈顶 pop，从 lastMutatedReceiver_ 取基对象执行索引赋值。", "a[i] = 1;"},
        {"OP_WRITEBACK_INDEX_LOCAL", "其他", "槽位(1B)", "弹出 1 个值",
         "索引写回到局部变量：索引从栈顶 pop，从 lastMutatedReceiver_ 取基对象执行索引赋值。", "a[i] = 1;"},
        ...
        {"OP_WRITEBACK_INDEX_UPVALUE", "其他", "upvalue索引(1B)", "弹出 1 个值",
         "索引写回到 upvalue：索引从栈顶 pop，从 lastMutatedReceiver_ 取基对象执行索引赋值。", "a[i] = 1;"},
        {"OP_LOAD_MUTATED", "其他", "无", "压入 1 个值",
         "将 lastMutatedReceiver_ 压入栈顶（不清除），供嵌套左值写回链使用。", "a.b.c = 1;"},
        {"OP_TRY_BEGIN", "其他", "catch偏移(2B)", "无效果",
         "注册 try 块的 catch 处理偏移。异常抛出时沿调用栈寻找匹配的 catch 并跳转到该处。",
         "try { ... } catch (e) { ... }"},
        {"OP_TRY_END", "其他", "无", "无效果",
         "try 块正常结束，弹出已注册的 catch 处理器。try 块未抛异常时走此路径清理。", "try { ... } catch (e) { ... }"},
        {"OP_THROW", "其他", "无", "弹出 1 个值",
         "弹出栈顶值作为异常对象并抛出，沿调用栈寻找匹配的 catch。catch 变量作用域需正确隔离。", "throw \"error\";"},
...
uint16_t readShortOperand(const BytecodeChunk& chunk, size_t ip) {
    if (ip + 2 < chunk.code.size()) {
        return static_cast<uint16_t>(chunk.code[ip + 1]) | static_cast<uint16_t>(chunk.code[ip + 2] << 8);
    }
    return 0;
}
...
QString describeOperands(const BytecodeChunk& chunk, size_t ip, OpCode op) {
    ...
    case OpCode::OP_MEMBER_SET_VAR: {
        ...
        uint16_t fieldIdx =
            (ip + 4 < chunk.code.size()) ? static_cast<uint16_t>(chunk.code[ip + 3] | (chunk.code[ip + 4] << 8)) : 0;
    ...
    case OpCode::OP_WRITEBACK_MEMBER_UPVALUE: {
        ...
        uint16_t fieldIdx =
            (ip + 3 < chunk.code.size()) ? static_cast<uint16_t>(chunk.code[ip + 2] | (chunk.code[ip + 3] << 8)) : 0;
...
QString StepExplainerLibrary::generateStepExplanation(const BytecodeChunk& chunk, size_t ip, OpCode op) {
    ...
    QString html = QStringLiteral("<h3>%1</h3>"
...
StepExplainerPanel::StepExplainerPanel(QWidget* parent) : QWidget(parent) {
    ...
    // 预填示例代码
    sourceEdit_->setText(
        QString::fromUtf8("var x = 42;\nvar y = 10;\nvar z = x + y;\nif (z > 40) {\n    print(z);\n}\n"));

    populateOpCodes();

    // 占位提示
    stepDetail_->setHtml(
        tr("<div style='color:#6E6E6E; "
           "padding:8px;'><i>（点击「编译并生成讲解」后，选中上方表格的某行查看该指令的详细讲解）</i></div>"));
}
...
void StepExplainerPanel::buildExplainPage(QWidget* host) {
    ...
    connect(stepTable_, &QTableWidget::currentCellChanged, [this](int, int, int, int) { onStepSelected(); });
...
void StepExplainerPanel::onCompile() {
    ...
    if (source.empty()) {
        stepDetail_->setHtml(tr("<div style='color:#C0392B; padding:8px;'><b>请输入源码</b></div>"));
        return;
    }
    ...
    try {
        ...
        stepDetail_->setHtml(tr("<div style='color:#C0392B; padding:8px;'><b>词法错误:</b> %1</div>")
    ...
    try {
        ...
        stepDetail_->setHtml(tr("<div style='color:#C0392B; padding:8px;'><b>语法错误:</b> %1</div>")
                                 .arg(QString::fromUtf8(e.what()).toHtmlEscaped()));
        return;
    }
    if (parser.hasErrors()) {
    ...
    try {
        ...
        stepDetail_->setHtml(tr("<div style='color:#C0392B; padding:8px;'><b>编译错误:</b> %1</div>")
                                 .arg(QString::fromUtf8(e.what()).toHtmlEscaped()));
        return;
    }
    ...
    if (!steps_.empty()) {
        ...
        stepDetail_->setHtml(tr("<div style='color:#6E6E6E; padding:8px;'><i>（编译产物为空）</i></div>"));
    }
}
