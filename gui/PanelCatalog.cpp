    ...
    static const std::vector<PanelCategory> kCategories = {
        ...
        {"编译前端",
         "🔤",
         {
             ...
             {"ast-editor", "AST 可视化编辑器", "🎨"},
             {"lint-explorer", "静态分析探索器", "🔍"},
         }},
        {"执行引擎",
         "⚙️",
         {
             {"backend-compare", "三后端对比", "⚖️"},
             {"jit-visualizer", "JIT 编译可视化", "⚡"},
             {"vm-sandbox", "VM 沙盒", "🎮"},
             {"memory-model", "内存模型", "🧠"},
             {"ir-transform", "IR 优化回放", "✨"},
             {"bytecode-trace", "字节码追踪", "📡"},
             {"step-explainer", "执行步骤讲解", "📝"},
             {"backend-parallel", "三后端并行可视化", "🔀"},
             {"inline-cache", "内联缓存可视化", "💡"},
             {"loop-unrolling", "循环展开可视化", "🔁"},
             {"escape-analysis", "逃逸分析可视化", "🚪"},
             {"register-allocator", "寄存器分配可视化", "🎯"},
             {"performance-race", "三后端性能竞赛", "🏁"},
             {"memory-layout", "内存布局可视化", "🧠"},
             {"gc-visualizer", "GC 垃圾回收可视化", "🗑️"},
             {"call-stack", "调用栈检查器", "📚"},
        ...
        {"深入实战",
         ...
         {
             {"bug-hunt", "Bug 狩猎", "🐛"},
             {"fuzz-playground", "模糊测试游乐场", "🎲"},
             {"exception-flow", "异常流可视化", "⚡"},
             {"closure-inspector", "闭包检查器", "🔒"},
             {"profile-dashboard", "性能仪表盘", "📊"},
             {"exercise-grader", "交互式练习评分", "📝"},
             {"module-system", "模块系统可视化", "📦"},
             {"lab-manual", "实验手册", "📘"},
         }},
    };
    return kCategories;
