...
const std::vector<LearningActivity>& LearningPathData::activities() {
    static const std::vector<LearningActivity> kActivities = {
        ...
        LearningActivity{"welcome",
                         ...
                         ActivityType::TOY,
                         "wave"},
        LearningActivity{"code-journey",
                         "🚀 代码生命旅程",
                         "📍 一张静态信息图，把 print(1+2*3) "
                         "从敲下源码到冒出结果的全过程摊给你看：词法→语法→字节码→执行。点进去看即算完成。",
                         0,
                         {},
                         5,
                         ActivityType::TOY,
        ...
        LearningActivity{
            ...
            ActivityType::TOY,
            "book"},
        LearningActivity{"visited-pipeline",
                         "🔗 参考资料：编译管线可视化",
                         ...
                         ActivityType::TOY,
                         "link"},
        LearningActivity{"visited-memory-model",
                         "🧠 参考资料：内存模型动画",
                         "📍 内存长啥样？这里用动画讲 NaN-boxing 的 8 字节 Value 布局、mark-sweep "
                         "垃圾回收的分步过程，还附 6 种堆对象的说明。",
                         0,
                         {},
                         5,
                         ActivityType::TOY,
                         "brain"},
        LearningActivity{"visited-bytecode-trace",
                         "📡 参考资料：字节码追踪",
                         ...
                         ActivityType::TOY,
                         "signal"},
        LearningActivity{"visited-exception-flow",
                         "⚡ 参考资料：异常流可视化",
        ...
        // ==================== 阶段一：编译前端 ====================
        LearningActivity{"lab-01",
                         "📖 实验 1：词法分析",
                         "📍 词法分析这一关：亲手把源码切成带类型的 Token。看清 Lexer "
                         "的状态机是怎么扫的、为啥认「最长匹配」，注释又是怎么被单独剔掉的。",
                         1,
                         {"token-puzzle"},
                         30,
                         ActivityType::LAB,
                         "lab"},
        LearningActivity{"lab-02",
                         "📖 实验 2：递归下降解析",
                         "📍 语法分析这一关：用递归下降把 Token 串拼成 AST。拿 1+2*3 和 (1+2)*3 "
                         "对照，优先级和左/右结合性怎么塑造树形，一下就有感觉了。",
                         1,
                         {"lab-01", "ast-toy"},
                         45,
                         ActivityType::LAB,
        ...
        LearningActivity{"syntax-explorer",
                         ...
                         ActivityType::TOY,
                         "book"},
        LearningActivity{"op-priority-challenge",
                         "🏆 挑战：运算符优先级链",
        ...
        // ==================== 阶段二：执行引擎 ====================
        LearningActivity{"lab-03",
                         "📖 实验 3：树遍历解释器",
                         "📍 语义分析 + 解释执行：看编译器怎么查作用域、类型、定义和参数个数，再用 Visitor 模式遍历 "
                         "AST，把程序真正跑起来。",
                         2,
                         {"lab-02", "vm-sandbox"},
                         60,
                         ActivityType::LAB,
                         "lab"},
        LearningActivity{"lab-04",
                         "📖 实验 4：栈式字节码 VM",
                         "📍 目标代码生成：把 AST 翻成栈式字节码，再在 StackVM 上单步跑。每条指令怎么 push/pop "
                         "操作数栈，看得清清楚楚。",
                         2,
                         {"lab-03"},
                         60,
                         ActivityType::LAB,
                         "lab"},
        LearningActivity{"lab-05",
                         "📖 实验 5：寄存器式 VM + IR",
                         "📍 中间代码 + 优化：弄懂 IR 三地址码和 SSA "
                         "虚拟寄存器，看常量折叠、复制传播、死代码消除怎么给代码瘦身，最后 lowering 到寄存器式 VM。",
                         2,
                         {"lab-04"},
                         75,
                         ActivityType::LAB,
        ...
        // ==================== 阶段三：深入理解 ====================
        LearningActivity{"lab-06",
                         "📖 实验 6：三后端一致性",
                         "📍 三后端一致性：亲手验证三条路径对同一份源码结果必须一样。整数除法截断、and-or "
                         "短路这些统一语义，到底意味着什么。",
                         3,
                         {"lab-05", "backend-compare"},
                         45,
                         ActivityType::LAB,
                         "lab"},
        LearningActivity{"lab-07",
                         "📖 实验 7：内存模型",
                         "📍 内存模型：搞懂 NaN-boxing 的 8 字节 Value、COW 写时复制和 mark-sweep "
                         "GC。共享的数组为什么等到真要写时才分家，看完就明白。",
                         3,
                         {"lab-05"},
                         60,
                         ActivityType::LAB,
        ...
        LearningActivity{"ir-transform",
                         ...
                         ActivityType::TOY,
                         "git-commit"},
        LearningActivity{"profile-dashboard",
                         "📊 性能剖析仪表盘",
        ...
        LearningActivity{"lab-08",
                         "📖 实验 8：Bug 狩猎方法论",
                         "📍 Bug 狩猎方法论实战：读真实项目历史 Bug "
                         "的根因分析，练出「预测→对比→定位」的排错直觉，还有三后端对称审计的眼光。",
                         4,
                         {"lab-06"},
                         45,
                         ActivityType::LAB,
                         "lab"},
        LearningActivity{"bug-hunt-beginner",
                         "🐛 Bug 狩猎：入门级",
                         ...
                         ActivityType::CHALLENGE,
                         "bug"},
        LearningActivity{"bug-hunt-intermediate",
                         "🐛 Bug 狩猎：进阶级",
                         "📍 4 道进阶题，全来自真实项目 Bug：默认参数、RegisterVM、REPL、Formatter。动真格的了。",
                         4,
                         {"bug-hunt-beginner", "lab-07"},
                         60,
                         ActivityType::CHALLENGE,
                         "bug"},
        LearningActivity{"bug-hunt-expert",
                         "🐛 Bug 狩猎：专家级",
                         "📍 6 道硬核题：IR 栈泄漏(P0)、常量池去重(P1)、闭包 "
                         "upvalue(P1)、调试器行号(P1)、嵌套索引栈泄漏(P1)、模块路径(P2)。敢不敢挑战？",
                         4,
                         {"bug-hunt-intermediate", "ir-transform"},
                         90,
                         ActivityType::CHALLENGE,
                         "bug"},
        ...
        LearningActivity{"freeform-project",
                         ...
                         4,
                         {"bug-hunt-intermediate"},
                         180,
                         ActivityType::FREEFORM,
                         "rocket"},
    };
    return kActivities;
}
