...
GlossaryPanel::GlossaryPanel(QWidget* parent) : QWidget(parent) {
    ...
    searchEdit_->setStyleSheet(
        QString("QLineEdit { border: 1px solid %1; border-radius: 4px;"
                "  padding: 2px 8px; background: %2; selection-background-color: %3; }"
                "QLineEdit:focus { border-color: %3; }")
            .arg(TeachingTheme::border().name(), TeachingTheme::surface().name(), TeachingTheme::primary().name()));
    ...
    connect(searchEdit_, &QLineEdit::textChanged, this, &GlossaryPanel::onSearchChanged);

    // 列表选中 → 显示详情 + emit termActivated
    connect(listWidget_, &QListWidget::currentRowChanged, this, &GlossaryPanel::onTermSelected);
    ...
    connect(detailView_, &QTextBrowser::anchorClicked, this, [this](const QUrl& url) {
        ...
        if (!full.startsWith(prefix, Qt::CaseInsensitive))
            return;
...
void GlossaryPanel::loadEntries() {
    entries_ = {
        ...
        {"ast",
         ...
         "AST（Abstract Syntax Tree）就是源码被 Parser "
         "消化后长出的那棵树。它不管你敲键盘的先后，只记一件事：哪个语法构造包着哪个、谁得先算。\n\n"
         "**💡 生活类比**：AST 像家谱图或文件夹树——它不记录你敲键盘的顺序，只记录「谁包含谁、谁先算谁」。同一个算式 "
         "`1+2*3` 和 `1+(2*3)` 在 AST 里形状不同，因为树表达的是计算优先级。\n\n"
         ...
         "MiniLang 的 AST 节点继承自 `ASTNode` 基类，采用 intrusive 引用计数管理。三后端共享同一份 AST：Interpreter "
         "直接遍历执行，Compiler/IRBuilder 将其翻译为字节码或 IR。\n\n"
        ...
        {"bound-instance",
         ...
         "boundInstance_ 是 Environment 里的一个小抄：记下当前这个方法调用到底挂在哪个对象（this）上。执行 "
         "`obj.method()` 时，它就派上用场：\n\n"
        ...
        {"bytecode",
         ...
         "bytecode 是 Compiler 把 AST 翻译出来的一叠超简指令，交给 StackVM 逐张照做。每条指令就是 1 字节 OpCode "
         "加一截可变长度的操作数——短小，但够用。\n\n"
         "**💡 生活类比**：bytecode 是 MiniLang 的「目标代码」——像一组极简操作卡片（取 1、取 2、相加、打印），VM "
         "照着一张张翻做。它比 AST 更接近机器，却仍运行在虚拟机上。\n\n"
         "**指令分类**（~60 OpCode）：\n\n"
         "| 类别 | 示例 | 说明 |\n"
         "|------|------|------|\n"
        ...
        {"closure",
         ...
         "closure 就是一只攥着外层变量的函数。在 MiniLang 里，所有函数骨子里都是闭包——哪怕它啥也没捕获，也照样被 "
         "ClosureData 包了一层。\n\n"
         "**捕获机制**：\n"
        ...
        {"closure-data",
         ...
         "- `name`：函数名（用于错误报告与调试）\n"
         "- `arity`：参数数量（含默认参数处理）\n\n"
         "**创建时机**：执行 `OP_CLOSURE` 指令时，从 BytecodeChunk 中读取 upvalue 描述，逐一捕获当前 Environment "
         "中的变量。\n\n"
         "**调用语义**：调用 closure 时创建新栈帧，绑定参数到局部变量，将 upvalues 注入到帧 Environment 中。\n\n"
         "**GC 关系**：ClosureData 是 GC 管理的堆对象，引用了 BytecodeChunk 与 upvalue 目标变量，形成 GC "
         "图中的可达性路径。",
        ...
        {"cow",
         ...
         "COW（Copy-On-Write）是 MiniLang 给数组和字典用的一招省内存技巧：多个 Value "
         "先指着同一份堆数据，相安无事；直到其中一个要动手改，才真去拷贝一份独立副本。\n\n"
         "**工作流程**：\n\n"
        ...
        {"environment",
         ...
         "Environment 就是 MiniLang 管「变量在哪」的那条链子。它用链表实现：每个块、每个函数帧都有一个 Environment "
         "节点，靠 parent 指针一环扣一环，往外就是更大的作用域。\n\n"
         "**结构**：\n"
        ...
        {"error-recovery",
         ...
         "## Error Recovery 错误恢复\n\n写代码谁没手滑过？这个机制让 Parser "
         "就算踩到坑，也先跨过去把后面的问题一并揪出来，而不是一错就躺。\n\n"
        ...
        {"field-slot",
         ...
         "fieldSlotIndex "
         "把类实例的字段压成了一排连续编号的格子。顺着继承链展平：父类的字段占前面的号，子类的接在后面，于是访问字段就"
         "是按号直取，不用一层层爬。\n\n"
         "**布局示例**：\n\n"
         "```\n"
         ...
         "```\n\n"
         "**编译期计算**：fieldSlotIndex 在 Parser 处理类声明时确定，存入字段元数据。运行时通过 "
         "`instance.fields[slot]` 直接索引访问。\n\n"
         "**已知 Bug 模式**：\n"
        ...
        {"gc",
         ...
         "GC（Garbage Collector）是 MiniLang 的自动内存管家，用的就是 mark-sweep 算法。GcManager "
         "会定期扫一遍堆：从根集合出发能摸到的都留着，摸不到的，清掉。\n\n"
         "**工作流程**：\n\n"
         "1. **标记阶段**（mark）：从根集合（VM 操作数栈、Environment 链、全局变量、闭包 "
         "upvalues）出发，深度优先遍历所有可达堆对象，打上标记\n"
         ...
         "**与 RefCounted 关系**：MiniLang 同时使用引用计数与 GC——RefCounted 处理大多数引用语义释放，GC "
         "兜底处理循环引用。",
        ...
        {"integer-division",
         ...
         "MiniLang 的整数除法走的是「截断向零」的路子，跟 C/C++ 一个脾气：`7 / 2 = 3`，`-7 / 2 = -3`（注意不是 "
         "-4）。这点三后端必须一致，否则结果就对不上。\n\n"
         "**三后端统一**：\n"
         "- Interpreter：直接使用 C++ `/` 运算符（int 类型，自然截断向零）\n"
         "- StackVM：`OP_DIVIDE` 指令对 int 操作数执行 C++ `/`\n"
         "- RegisterVM：寄存器除法指令同样截断向零\n\n"
         "**历史 Bug**：曾出现 RegisterVM 使用浮点除法再取整导致负数结果不一致（向负无穷取整而非向零）。修复后统一为 "
         "C++ int `/` 语义。\n\n"
        ...
        {"ir",
         ...
         "IR（Intermediate Representation）是 MiniLang 架在 AST 和字节码中间的那一层。它长成 "
         "SSA-like（静态单赋值）的样子，用虚拟寄存器记账，好处是后面做优化时特别顺手。\n\n"
         "**💡 生活类比**：IR 像「世界语写的菜谱」——比 AST "
         "具体（已是步骤指令），又不绑定任何料理机（栈式/寄存器式都能翻译）。引入 "
         "IR，优化和翻译就只需写一次，三后端共享。\n\n"
         "**🔍 直观演示**：\n"
         "```\n"
         "源码:  var y = 1 + 2;\n"
         "IR:    v0 = LOAD_CONST 1\n"
         ...
         "**双后端 lowering**：IR 可 lowering 到 StackVM bytecode（BytecodeIRBackend）或 RegisterVM "
         "RegOp（RegisterBytecodeBackend）。",
        ...
        {"lexer",
         ...
         "Lexer 是编译管线的第一道工序，干的事是把源码字符流扫成 Token 序列。它是手写的有限状态机（不是 lex/yacc "
         "生成）——老派，但好调。\n\n"
         "**💡 生活类比**：Lexer "
         "像工厂流水线的「分拣机」——原料（字符）从一头进去，另一头出来的是按种类分好类的零件（Token）。它不关心零件怎么"
         "组装，只负责「认出这是什么」。\n\n"
         "**扫描规则**：\n"
         "- 跳过空白与注释（`//` 行注释、`/* */` 块注释）\n"
         "- 识别关键字（var / fun / class / if / while / for / return / break / continue / and / or / not / true / "
         "false / null / try / catch / finally / throw / import / export / extends / super）\n"
         "- 识别标识符（字母/下划线开头）\n"
         "- 识别数字字面量（整数 / 浮点 / 负号在 Parser 处理）\n"
         "- 识别字符串字面量（含插值 `\"...{expr}...\"`）\n"
         "- 识别运算符与分隔符\n\n"
         "**插值处理**：字符串插值 `\"hello {name}\"` 由 Lexer 拆分为多段——字符串字面量 + 表达式 Token 序列 + "
         "字符串字面量，嵌套深度限制 64 层。\n\n"
        ...
        {"lowering",
         ...
         "lowering 是编译器圈的黑话，意思就是把高层的 IR 翻译成更贴地的目标代码。在 MiniLang 里，IR 节点会被 lowering "
         "到两种后端：\n\n"
         "**💡 生活类比**：lowering 像「把世界语菜谱翻译成某品牌料理机的专属卡片」——高层 IR 是通用描述，lowering "
         "成栈式或寄存器式字节码，就是两种不同料理机的「执行卡片」。\n\n"
        ...
        {"module-loader",
         ...
         "## Module Loader 模块加载器\n\n你写一句 "
         "import，真正去翻文件、读源码、防重复加载的，就是这位。循环导入它用延迟加载处理。\n\n"
        ...
        {"module-system",
         "module system 模块系统",
         "运行时",
         "让你把代码拆到不同文件再用 import/export 拼回来的机制，还管着路径安全、循环导入延迟加载这些脏活。",
         ...
         "**循环导入延迟加载（P2-14）**：加载模块时记录正在加载的路径，若再次遇到同一路径则返回已创建的部分 env，"
         "不再报错。模块环境立即入缓存，内容按执行顺序填充。访问未定义导出名时由 Environment::get "
         "抛\"未定义变量\"错误。"
         "语义参考 ES Modules + Python。\n\n"
         "**预扫描**：模块加载前先扫描 FunDecl / ExportStmt，建立符号表后再执行。这避免了“函数提升”语义——MiniLang "
         "函数无提升，必须先声明后使用。\n\n"
         "**已知 Bug 模式**：\n"
        ...
        {"nan-boxing",
         ...
         "NaN-boxing 是 MiniLang 表示 Value 的巧劲儿：所有类型都编码成 8 字节（和 double 一样宽），再借 IEEE 754 里 "
         "NaN 那段没人用的比特，藏进类型标签和真正的数据。\n\n"
         "**💡 生活类比**：NaN-boxing 像把糖、书、箱子都塞进统一规格的 8 "
         "号储物柜——小物件直接放进柜子，大物件只留一张「指向仓库的提货卡」。柜子规格统一，管理极高效，这就是 MiniLang "
         "用 8 字节装下所有值的秘诀。\n\n"
         "**编码布局**（8 字节 = 64 位）：\n\n"
         "```\n"
        ...
        {"opstack",
         ...
         "operand stack 是 StackVM 的心脏，说白了就是一个定长数组 `Value[1024]` 配一个栈顶指针 "
         "sp。指令不明说操作数在哪，全靠隐式 push/pop 来递来递去。\n\n"
         "**💡 生活类比**：操作数栈像一个「只从顶上拿放的托盘」——`OP_ADD` "
         "就是「从托盘顶端取两个、算完放回去」。栈平衡不变量保证了托盘不会越堆越高（泄漏）也不会被掏空（下溢）。\n\n"
         "**操作示例**：\n"
        ...
        {"parser",
         ...
         "**💡 生活类比**：Parser 像把一串「主谓宾」单词拼回句子结构树的工人。「猫 吃 "
         "鱼」不是三个孤立词，而是「吃(主语=猫, 宾语=鱼)」——Parser 把线性的 Token 串还原出程序的层次。\n\n"
        ...
        {"register-vm",
         ...
         "**💡 生活类比**：RegisterVM 像有「一排带编号的盒子 R0-R31」——食材分类放进固定盒子，指令直接说「把 R1 和 R2 "
         "相加放进 R3」，不用每次都往一个盘子上堆。更接近真实 CPU 的做事方式。\n\n"
         "**与 StackVM 对比**：\n\n"
         ...
         "- 更接近现代 CPU 架构（JIT 友好）\n\n"
         "**IR 直接 lowering**：IRModule 经 RegisterBytecodeBackend 直接 lowering 为 RegOp，无需经 StackVM "
         "bytecode。\n\n"
        ...
        {"ssa",
         ...
         "SSA（Static Single Assignment）是编译器里一种中间表示形式，铁律是：每个变量只能被赋值一次。MiniLang 的 IR "
         "走的是 SSA-like 的路子。\n\n"
         "**💡 生活类比**：SSA 像「每次给新盒子贴新标签」——变量 `x` 第一次赋值叫 `v1`，第二次叫 "
         "`v2`，绝不重用。这样「谁定义了谁、谁用了谁」一目了然，优化器找死代码、做常量折叠都变得简单。\n\n"
         "**特性**：\n"
         ...
         "**lowering**：SSA 形式 lowering 到 StackVM bytecode 时，vreg 映射到局部变量槽位；lowering 到 RegisterVM "
         "时映射到物理寄存器。",
        ...
        {"stack-vm",
         ...
         "**💡 生活类比**：StackVM "
         "像只用「一个盘子」做饭——所有食材都堆在那一个盘子上，取料从顶上拿、做好放回去。简单但频繁地「放/取」，就是 "
         "push/pop。\n\n"
         "**🔍 直观演示**：\n"
         "```\n"
         "OP_INT 1   ; 盘子: [1]\n"
        ...
        {"string-interpolation",
         ...
         "## String Interpolation 字符串插值\n\n想在字符串里顺手算点东西？这个特性让你把表达式嵌进引号里，Lexer 连套 "
         "64 层嵌套都不带喘。\n\n"
        ...
        {"token",
         ...
         "**💡 生活类比**：Token 就像句子里的「词」。我们说「我爱编程」，分词后得到「我 / 爱 / "
         "编程」三个有意义的词——编译器也是先把字符流切成「词」（Token），后面的语法分析才认得出结构。\n\n"
         "**🔍 直观演示**：\n"
         "```\n"
         "输入:  var x = 42;\n"
         "Token: var(关键字) / x(标识符) / =(运算符) / 42(数字) / ;(分隔符)\n"
         "```\n"
         "注：`42` 作为一个整体被识别为「数字」Token，而不是四个独立字符 `4` `2`——Token "
         "的最小单元是「有意义的词」，不是字符。\n\n"
         "- **类型**（TokenType）：关键字 / 标识符 / 数字 / 字符串 / 运算符 / 分隔符 / EOF\n"
         "- **字面量**（lexeme）：原始字符串（如 `\"hello\"` 整体作为一个 STRING token 的 lexeme）\n"
         "- **位置**（line / column）：行列号，用于错误报告与调试\n\n"
         "**类型分类**：\n"
         "- 关键字：var / fun / class / if / while / for / return / break / continue / and / or / not / true / false / "
         "null / try / catch / finally / throw / import / export / extends / super\n"
         "- 字面量：INT / FLOAT / STRING（含插值标记）\n"
         "- 标识符：IDENTIFIER\n"
         "- 运算符：PLUS / MINUS / STAR / SLASH / PERCENT / EQ / NE / LT / GT / LE / GE / ASSIGN / DOT / COMMA / "
        ...
        {"triple-backend",
         ...
         "三后端一致性是 MiniLang 的一条铁律：同一份源码，走 Interpreter、StackVM、RegisterVM "
         "三条路，语义结果必须一模一样。\n\n"
         "**💡 生活类比**：三后端一致性像「同一道菜谱，人工现做 / A 牌机 / B "
         "牌机三种做法，端上桌味道必须一模一样」。内部步骤可以天差地别，但结果必须一致——这是编译器正确性的金标准。\n\n"
         "**三条路径**：\n"
        ...
        {"type-annotation",
         ...
         "type annotation 是 MiniLang 里可选的类型标注，支持 `int` / `float` / `bool` / `string` / `null` "
         "这些类型。\n\n"
         "**语法**：\n"
         "```\n"
         "var x: int = 5;          // 变量注解\n"
...
void GlossaryPanel::populateList(const QString& filter) {
    ...
    for (int i = 0; i < n && i < entries_.size(); ++i) {
        ...
        if (!match) {
            ...
            match = e.term.toLower().contains(f) || e.id.toLower().contains(f) || e.category.toLower().contains(f) ||
...
void GlossaryPanel::showDetail(int index) {
    ...
    if (!e.related.isEmpty()) {
        ...
        for (const QString& rid : e.related) {
            ...
            oss << "<li><a href=\"term:" << rid.toStdString() << "\">" << display.toStdString() << "</a></li>";
        }
        oss << "</ul>";
    }
...
QString GlossaryPanel::relatedPanelFor(const QString& termId) {
    static const QHash<QString, QString> kTermToPanel = {
        ...
        // AUDIT-P2 fix: 补全 6 个遗漏术语的关联面板映射
        {QStringLiteral("debugger"), QStringLiteral("breakpoint-condition")},
        {QStringLiteral("line-info"), QStringLiteral("bytecode-trace")},
        {QStringLiteral("module-loader"), QStringLiteral("pipeline")},
        {QStringLiteral("output-capture"), QStringLiteral("vm-sandbox")},
        {QStringLiteral("error-recovery"), QStringLiteral("syntax-explorer")},
        {QStringLiteral("string-interpolation"), QStringLiteral("vm-sandbox")},
    };
    auto it = kTermToPanel.constFind(termId);
    if (it == kTermToPanel.constEnd())
        return QString();
    return it.value();
}
