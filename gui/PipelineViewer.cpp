...
QString PipelineViewer::stageTitle(int step) {
    static const QString kTitles[] = {
        QString::fromUtf8("源码 Source"), QString::fromUtf8("Token 词法"),      QString::fromUtf8("AST 语法树"),
        QString::fromUtf8("IR 中间表示"), QString::fromUtf8("字节码 Bytecode"),
    };
    if (step < 0 || step >= 5)
        return QString();
    return kTitles[step];
...
QString PipelineViewer::stageDesc(int step) {
    static const QString kDescs[] = {
        QString::fromUtf8("用户输入的 MiniLang 源代码文本"), QString::fromUtf8("词法分析器输出的 Token 序列"),
        QString::fromUtf8("语法分析器构建的抽象语法树"),     QString::fromUtf8("AST 转换的三地址码中间表示"),
...
PipelineViewer::PipelineViewer(QWidget* parent) : QWidget(parent) {
    ...
    auto makeArrow = [this](int destStep) {
        ...
        arrow->setStyleSheet(QString("QLabel {"
                                     "  color: %1;"
                                     "  font-size: 22px;"
                                     "  font-weight: bold;"
    ...
    tokenTable_->setHorizontalHeaderLabels({QString::fromUtf8("#"), QString::fromUtf8("类型"),
                                            QString::fromUtf8("字面文本"), QString::fromUtf8("行"),
                                            QString::fromUtf8("列")});
    ...
    auto wrapPage = [this](QWidget* content, int step) -> QWidget* {
        ...
        page->setStyleSheet(QString("QWidget#stagePage%1 { background: transparent; }").arg(step));
        ...
        const QString html =
            QString("<div style='font-family: \"Microsoft YaHei\",\"PingFang SC\",\"Noto Sans CJK SC\",\"Source Han "
                    "Sans SC\",\"Segoe UI\",\"SF Pro Text\",\"Arial Unicode MS\",\"Arial\",sans-serif;'>"
...
// AUDIT-P2 fix: 析构函数——反注册 vmStateChanged 监听器，避免 controller_ 持有悬垂回调
PipelineViewer::~PipelineViewer() {
    if (controller_) {
        controller_->removeVmStateChangedListener(this);
    }
}

// AUDIT-P2 fix: setController 注册 vmStateChanged 监听器，源码编辑后切页能拿到最新数据。
// 原实现是纯赋值，未注册任何监听器，导致源码页编辑后切到 Token/AST/IR/Bytecode 页
// 显示陈旧数据，用户误以为是最新代码的分析结果。
void PipelineViewer::setController(IdeController* controller) {
    // 反注册旧 controller 的监听器
    if (controller_) {
        controller_->removeVmStateChangedListener(this);
    }
    controller_ = controller;
    // AUDIT-P1 fix: 切换 controller 时重置缓存指纹，确保下次 populate 强制刷新
    lastTokensPtr_ = nullptr;
    lastIRPtr_ = nullptr;
    lastCompileResultPtr_ = nullptr;
    // 注册新 controller 的监听器，VM 状态变化时刷新当前页
    if (controller_) {
        controller_->addVmStateChangedListener(this, [this]() { reloadCurrentStep(); });
    }
}
...
/// force=true 时绕过指纹缓存，强制重新填充（switchToStep 等用户显式操作应传 true）。
void PipelineViewer::reloadCurrentStep(bool force) {
    forceRefresh_ = force;
    switch (currentStep_) {
    case 0:
        populateSource();
...
void PipelineViewer::updateStatusBar() {
    if (!statusLabel_)
        return;
    static const QStringList kStepNames = {QString::fromUtf8("源码"), QString::fromUtf8("Token"),
...
void PipelineViewer::populateTokens() {
    ...
    const auto& tokens = controller_->lastTokens();
    // AUDIT-P1 fix: 指纹缓存——vmStateChanged 高频触发时若 tokens 指针未变则跳过重建。
    // forceRefresh_=true（用户显式切换页面）时绕过缓存。
    if (!forceRefresh_ && &tokens == lastTokensPtr_)
        return;
    lastTokensPtr_ = &tokens;
...
void PipelineViewer::populateIR() {
    ...
    if (!ir) {
        irBrowser_->setHtml(QString::fromUtf8("<i>未启用 IR 编译。请在视图菜单启用 IR 模式或触发一次 IR 编译。</i>"));
        return;
    }
    // AUDIT-P1 fix: 指纹缓存——vmStateChanged 高频触发时若 IR 指针未变则跳过 IRToString。
    // forceRefresh_=true（用户显式切换页面）时绕过缓存。
    if (!forceRefresh_ && ir == lastIRPtr_)
        return;
    lastIRPtr_ = ir;
    // 简化输出：用 IRToString
...
void PipelineViewer::populateBytecode() {
    ...
    // AUDIT-P1 fix: 指纹缓存——vmStateChanged 高频触发时若 CompileResult 指针未变则跳过 disassemble。
    // forceRefresh_=true（用户显式切换页面）时绕过缓存。
    if (!forceRefresh_ && &result == lastCompileResultPtr_)
        return;
    lastCompileResultPtr_ = &result;
    std::ostringstream os;
    os << "=== main chunk ===\n";
