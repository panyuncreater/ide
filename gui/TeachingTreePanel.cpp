...
TeachingTreePanel::TeachingTreePanel(QWidget* parent)
    ...
    bannerTitle_->setStyleSheet(QStringLiteral(
        "font-size: 15px; font-weight: 600; color: white;"));
    bannerSubtitle_ = new QLabel(QString::fromUtf8("点击下方任意主题开始学习"), headerBanner_);
    bannerSubtitle_->setStyleSheet(QStringLiteral(
        "font-size: 11px; color: rgba(255,255,255,200);"));
    bannerLayout->addWidget(bannerTitle_);
    bannerLayout->addWidget(bannerSubtitle_);
    headerBanner_->setStyleSheet(QStringLiteral(
        "#teachingBanner {"
    ...
    connect(searchEdit_, &QLineEdit::textChanged,
            this, &TeachingTreePanel::onSearchChanged);
    ...
    connect(tree_, &QTreeWidget::itemClicked,
            this, &TeachingTreePanel::onItemClicked);
    // 键盘 Enter / 双击也触发（无障碍支持）
    connect(tree_, &QTreeWidget::itemActivated,
            this, &TeachingTreePanel::onItemActivated);
...
void TeachingTreePanel::buildTree() {
    ...
    for (int i = 0; i < tree_->topLevelItemCount(); ++i) {
        ...
        if (top && top->data(0, Qt::UserRole).toString() == QStringLiteral("category")) {
            ...
            if (++expandedCount >= 2) break;  // 只展开前两个 category
        }
    }
...
void TeachingTreePanel::onItemClicked(QTreeWidgetItem* item, int column) {
    ...
    if (id.isEmpty() || id == QStringLiteral("category")) return;
