...
std::string extractCommandName(const std::string& input) {
    ...
    if (i >= input.size() || input[i] != '%') return "";
    ++i; // 跳过 %
    size_t start = i;
    while (i < input.size() && input[i] != ' ' && input[i] != '\t' && input[i] != '\r' && input[i] != '\n') {
        ++i;
...
std::string handleTokens(IdeController* controller) {
    ...
    for (const auto& tok : tokens) {
        ...
        if (lex.size() > 20) lex = lex.substr(0, 17) + "...";
        os << lex;
...
std::string handleMemory(IdeController* controller) {
    ...
    auto countValue = [&counts](const Value& v) {
        counts[valueTypeName(v.getType())]++;
    };
...
std::string MagicCommands::handle(const std::string& input, IdeController* controller) {
    ...
    if (cmd.empty()) return "";  // 非 magic 命令

    if (cmd == "help")       return handleHelp();
    if (cmd == "version")    return handleVersion();
    if (cmd == "disassemble")return handleDisassemble(controller);
    if (cmd == "ir")         return handleIr(controller);
    if (cmd == "ast")        return handleAst(controller);
    if (cmd == "tokens")     return handleTokens(controller);
    if (cmd == "memory")     return handleMemory(controller);
    if (cmd == "profile")    return handleProfile(controller);
    if (cmd == "compare")    return handleCompare(controller);
    if (cmd == "reset")      return handleReset(controller);
