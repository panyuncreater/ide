    ...
    static const QHash<QString, HelpDoc> docs = {
        {
            ...
            {
                mlTr("一行 MiniLang 代码从出生到结果，要走过 6 个阶段：源码 → Token → AST → IR → 字节码 → 输出。这个面板就是把这趟旅程摊开给你看——每个阶段都带示例和跳转按钮，点一下就能去对应的专题面板动手玩。"),
                mlTr("1. 先在本面板把全流程溜一遍\n2. 看中哪个阶段，点底部按钮跳过去\n3. 在对应面板里真正动手试"),
        ...
        },
        {
            ...
            {
                mlTr("编译管线可视化：把源码变成运行结果的全过程拆成 6 个阶段，一步步演示给你看。每推进一步，都能看到这一步吃进去什么、吐出来什么（输入/输出数据结构和中间产物）。"),
        ...
        },
        {
            ...
            {
                mlTr("Token 拼图：拖着一个个 Token 把源码拼回去，直观感受词法分析器是怎么把字符流切成「有类型的零件」的。关卡通关后自动解锁下一关——像闯关，不像上课。"),
                mlTr("1. 先看「编译管线可视化」搞懂 Token 是啥\n2. 在本面板把拼图关卡刷了\n3. 顺便去「语法浏览器」看看 Token 类型全家福"),
        ...
        },
        {
            ...
            {
                mlTr("AST 构建器：拖节点搭出一棵抽象语法树，亲眼看看运算符优先级和结合性是怎么决定树长什么样的。树长歪了，结果就歪了——这面板让你手动把树掰正。"),
                mlTr("1. 先把「Token 拼图」做了，理解词法\n2. 在本面板动手搭 AST\n3. 回头对照「编译管线可视化」里的 AST 阶段"),
        ...
        },
        {
            ...
            {
                mlTr("语法浏览器：把 MiniLang 的所有语法结构和示例代码摊在一张表里。点一个语法节点，就能看到它对应的 AST 长相和典型用法——相当于一本会动的语法说明书。"),
        ...
        },
        {
            ...
            {
                mlTr("三后端对比：同一份代码，让 Interpreter / StackVM / RegisterVM 三条路径各跑一遍，把输出和耗时摆在一起比。哪一后端偷偷耍了花招，一眼就看出来——这正是 MiniLang 的「金标准」一致性检查。"),
                mlTr("1. 先搞懂三后端架构（看「代码生命旅程」）\n2. 在本面板输入代码对比三份输出\n3. 万一结果对不上，八成是 Bug，去「Bug 狩猎」练手"),
        ...
        },
        {
            ...
            {
                mlTr("VM 栈沙盒：自己动手 push 字节码、单步执行，看着操作数栈一会儿压进去、一会儿弹出来。栈式虚拟机就这么点事儿——把指令一条条喂进去，看栈怎么变。"),
                mlTr("1. 先看「编译管线可视化」的字节码阶段\n2. 在本面板手动摆弄栈\n3. 配合「字节码追踪」看真实代码跑起来后的轨迹"),
        ...
        },
        {
            ...
            {
                mlTr("内存模型可视化：把 NaN-boxing 编码、写时复制（COW）容器、GC 的 mark-sweep 全做成了实时动画。值到底在内存里怎么放、怎么共享、怎么回收，看动画比看文档直观十倍。"),
        ...
        },
        {
            ...
            {
                mlTr("IR 优化回放：把 const-fold / DCE / copy-prop / CSE / loop-unroll 这五种优化 pass 一步步演给你看，每步都解释「为什么这么改」。编译器不是魔法，是一步步把代码变瘦变快。"),
                mlTr("1. 先看「编译管线可视化」的 IR 阶段\n2. 在本面板逐步推进优化\n3. 配合「字节码追踪」看优化后的字节码长啥样"),
        ...
        },
        {
            ...
            {
                mlTr("字节码追踪：指令级执行回放，每条字节码跑完都记一下操作数栈、寄存器、IP 的状态。想搞清楚「这条指令到底改了什么」，就靠它来回溯。"),
        ...
        },
        {
            ...
            {
                mlTr("调用栈检查器：把函数调用时的栈帧结构、参数怎么传、返回地址在哪，全画了出来。函数一层套一层时，谁调用了谁、返回去哪儿，一目了然。"),
                mlTr("1. 先搞懂函数调用机制\n2. 在本面板看调用栈怎么长怎么消\n3. 配合「变量检查器」看局部变量"),
        ...
        },
        {
            ...
            {
                mlTr("变量检查器：把作用域链、闭包捕获、变量生命周期画出来。一个变量在哪个作用域看得见、活多久，这个面板替你盯着。"),
        ...
        },
        {
            ...
            {
                mlTr("条件断点可视化：把断点条件的求值沙箱和命中次数摆出来。条件断点到底什么时候算、什么时候命中，不是玄学——这个面板把它摊开给你看。"),
                mlTr("1. 先搞懂基本断点怎么用\n2. 在本面板试条件表达式\n3. 去编辑器里设个真实的条件断点验证"),
                mlTr("条件断点 / 求值沙箱 / 命中计数")
            }
        },
        {
            QStringLiteral("bug-hunt"),
            {
                ...
                mlTr("Bug 狩猎：三档分级挑战（初阶 / 中阶 / 高阶），每一档都配三后端对比 + 变体题。读代码找 Bug 的直觉，是刷出来的——这面板就是你的训练场。"),
                mlTr("1. 基础学完再进来\n2. 一道道 Bug 题啃过去\n3. 开「变体模式」挑战进阶变体"),
        ...
        },
        {
            ...
            {
                mlTr("异常流可视化：把 try/catch/finally 的传播路径和栈效应画出来。异常抛出后怎么沿着调用栈往上爬、最后在哪被抓住，看一眼就明白。"),
        ...
        },
        {
            ...
            {
                mlTr("闭包检查器：把 upvalue 的一生（capture / heap / access / close / destroy）画出来。闭包到底是怎么抓住外面那个变量、又怎么一直抱着不放的，这个面板说清楚了。"),
        ...
        },
        {
            ...
            {
                mlTr("性能仪表盘：把三后端的耗时摆一起比，再列个 opcode 执行次数 Top 10 热点。你的代码慢在哪、该从哪优化，看这张图就有方向了。"),
                mlTr("1. 基础学完再进来\n2. 对比三后端谁快谁慢\n3. 盯着 opcode 热点找优化点"),
        ...
        },
        {
            ...
            {
                mlTr("术语表：把 MiniLang IDE 的核心术语（词法 / 语法 / IR / 字节码 / VM / 内存模型……）按类归在一起，每条都配简明释义和跳转按钮。碰到不认识的概念，随时来翻。"),
...
TeachingPanelHeader::TeachingPanelHeader(const QString& panelId,
    ...
    connect(tourBtn_, &PushButton::clicked, this, [this]() {
        emit guidedTourRequested(panelId_);
    });
...
void TeachingPanelHeader::showHelpDialog() {
    ...
    const QString kTipLine = QStringLiteral(
        "<hr style='border: none; border-top: 1px dashed #93A1A1; margin: 14px 0 8px 0;'/>"
    ...
    const QString kDialogQss = QStringLiteral(
        "QDialog#helpDialog {"
        "  background: #FDF6E3;"
        "}"
...
void TeachingPanelHeader::adjustHelpDialogSize(QDialog* dlg, QTextBrowser* browser) {
    if (!dlg || !browser || !browser->document()) return;

    // 强制布局计算，确保 viewport 拿到有效宽度
    if (auto* lay = dlg->layout()) lay->activate();
    ...
    if (viewportWidth <= 0) viewportWidth = 600;  // 最终兜底
