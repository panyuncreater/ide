...
TeachingSubPageBar::TeachingSubPageBar(QWidget* parent) : QWidget(parent) {
    ...
    // UX-R2 fix: 按钮间距 4→6px，减少视觉拥挤感
    buttonBar_->setSpacing(6);
...
void TeachingSubPageBar::updateButtonStyles() {
    ...
    // UX-R2 fix: 选中态增加底部 3px 主题色指示条（现代 tab 指示器风格），
    //            未选中态底部透明，增强选中态辨识度。
    ...
    const QString primaryStyle =
        QString("QPushButton { background: %1; color: %2; border: 1px solid %1; "
                "border-bottom: 3px solid %1; border-radius: 6px 6px 0 0; padding: 7px 16px; font-weight: bold; }"
    ...
    const QString secondaryStyle =
        QString("QPushButton { background: %1; color: %2; border: 1px solid %3; "
                "border-bottom: 3px solid transparent; border-radius: 6px 6px 0 0; padding: 7px 16px; "
                "font-weight: 500; }"
