...
std::string ErrorHintEngine::enrichErrorMessage(const std::string& msg,
    ...
    if (msg.find("Unexpected token") != std::string::npos ||
        msg.find("意外的 token") != std::string::npos ||
    ...
    // N3 fix: 递归深度超限 / 栈溢出
    if (msg.find("Maximum recursion depth") != std::string::npos ||
        msg.find("recursion") != std::string::npos ||
        msg.find("stack overflow") != std::string::npos ||
        msg.find("递归") != std::string::npos) {
    ...
    if (msg.find("null") != std::string::npos &&
        (msg.find("property") != std::string::npos ||
         msg.find("method") != std::string::npos ||
         msg.find("access") != std::string::npos ||
         msg.find("属性") != std::string::npos)) {
...
const std::vector<ErrorHintEngine::ErrorPattern>& ErrorHintEngine::errorPatterns() {
    static const std::vector<ErrorPattern> kPatterns = {
        ...
        {"index-out-of-bounds", "数组索引越界",
         "var arr = [1, 2];\nprint(arr[5]);\n", "runtime"},
        {"type-mismatch",       "类型不匹配（int 注解拒绝 string）",
         "var x: int = \"string\";\nprint(x);\n", "type"},
        {"unexpected-token",    "无法识别的语法结构",
         "= 5;\n", "parser"},
        {"arity-mismatch",      "函数参数个数不匹配",
         "fun add(a, b) { return a + b; }\nprint(add(1));\n", "runtime"},
        {"recursion-depth",     "递归深度超限（无限递归）",
         "fun f() { f(); }\nf();\n", "runtime"},
        {"null-access",         "null 值上访问属性/方法",
         "var x = null;\nprint(x.field);\n", "runtime"},
    };
    return kPatterns;
}
