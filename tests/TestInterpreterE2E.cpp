...
TEST(InterpreterE2E, TypeAnnotationsSampleCheck) {
    ...
    {
        Lexer lexer;
        auto tokens = lexer.scan("fun applyFormatter(fun(int):string cb, int value) { return cb(value); }"
    ...
    // 5b) 真正执行
    std::string src5 = "fun applyFormatter(fun(int):string cb, int value) { return cb(value); }"
                       "fun numberName(int n): string { return \"num\" + str(n); }"
                       "print(applyFormatter(numberName, 7));";
...
TEST(InterpreterE2E, DictLen) {
    std::string src = "var d = {\"a\": 1, \"b\": 2, \"c\": 3};"
                      "print(d.len());";
    EXPECT_EQ(runInterpreterOutput(src), "3");
}
...
TEST(InterpreterE2E, DictHas) {
    std::string src = "var d = {\"name\": \"Alice\"};"
                      "print(d.has(\"name\"));"
                      "print(d.has(\"age\"));";
    EXPECT_EQ(runInterpreterOutput(src), "truefalse");
}
...
TEST(InterpreterE2E, StringConcatenation) {
    EXPECT_EQ(runInterpreterOutput("print(\"hello\" + \" \" + \"world\");"), "hello world");
}
...
TEST(InterpreterE2E, ComprehensiveStringProcessing) {
    std::string src = "var s = \"hello,world,foo,bar\";"
                      "var parts = s.split(\",\");"
                      "print(parts.len());"
                      "var i = 0;"
                      "while (i < parts.len()) {"
...
static std::string runInterpreterWithModules(const std::string& source,
...
static bool runInterpreterWithModulesThrows(const std::string& source,
                                            const std::unordered_map<std::string, std::string>& modules) {
    Lexer lexer;
    auto tokens = lexer.scan(source);
    Parser parser;
...
TEST(InterpreterE2E, ModuleImportNamedNotExported) {
    std::string src = "import { secret } from \"m\";"
                      "print(secret);";
    std::unordered_map<std::string, std::string> modules = {{"m", "var secret = 42; export var public_val = 1;"}};
...
// 测试：循环导入延迟加载（P2-14）
// 循环依赖不再报错——模块按执行顺序加载，回路闭合时返回部分 env。
...
TEST(InterpreterE2E, ModuleCircularDependency) {
    ...
    EXPECT_EQ(runInterpreterWithModules(src, modules), "");
}

// 测试：循环导入执行顺序（P2-14）
// 验证循环回路闭合时模块按导入顺序执行，print 输出反映执行序
TEST(InterpreterE2E, ModuleCircularDependencyExecutionOrder) {
    std::string src = "import \"a\";"
                      "print(\"main\");";
    std::unordered_map<std::string, std::string> modules = {{"a", "import \"b\"; print(\"a\");"},
                                                            {"b", "import \"a\"; print(\"b\");"}};
    // 执行序：b（回路闭合后先执行完）→ a → main
    EXPECT_EQ(runInterpreterWithModules(src, modules), "bamain");
}

// 测试：循环导入——导出名在循环引用前定义（P2-14）
// a 先 export get_a()，再 import "b"；b 循环 import "a" 时 get_a 已定义
TEST(InterpreterE2E, ModuleCircularDependencyExportBeforeCycle) {
    ...
    EXPECT_EQ(runInterpreterWithModules(src, modules), "b1");
}
...
TEST(InterpreterE2E, ModuleIsolation) {
    std::string src = "import { public_val } from \"m\";"
                      "print(public_val);";
    std::unordered_map<std::string, std::string> modules = {{"m", "var hidden = 999;"
...
TEST(InterpreterE2E, ModuleImportAllOnlyExports) {
    std::string src = "import \"m\";"
                      "print(private_val);";
    std::unordered_map<std::string, std::string> modules = {{"m", "var private_val = 999;"
                                                                  "export var public_val = 1;"}};
...
TEST(InterpreterE2E, ModuleImportNamedAtomicNoPollution) {
    ...
    EXPECT_TRUE(runInterpreterWithModulesThrows("import { a, nonexistent } from \"m\";"
                                                "print(a);",
                                                modules));
}
...
TEST(InterpreterE2E, ModuleCacheKeyNormalization) {
    std::string src = "import \"./mod.mini\";"
                      "import \"mod.mini\";"
                      "print(\"ok\");";
    std::unordered_map<std::string, std::string> modules = {{"mod.mini", "export var val = 1;"}};
...
TEST(InterpreterE2E, ModuleCacheKeyBackslashNormalization) {
    ...
    std::string src = "import \"sub/mod.mini\";"
                      "import \"sub\\\\mod.mini\";"
                      "print(\"ok\");";
    std::unordered_map<std::string, std::string> modules = {{"sub/mod.mini", "export var val = 1;"}};
