...
- **第八十八轮（2026-07-10）**：跨机器字体一致性（UI × 1）。彻底解决 IDE 字体在其他机器上"看不清"的问题。根因：`main.cpp` 全局字体 `setPixelSize(14)` + 短回退链 `{"Microsoft YaHei", "PingFang SC", "Segoe UI"}`，缺中文字体的机器中文 UI 回退到非中文字体显示不清；`monospaceFont()` 硬编码 `QFont("Consolas")` 无回退链；7 处面板绕过共享工厂。修复：`GuiTextUtils.h` 新增 `uiFont()`/`monospaceFont()` 统一字体工厂，用 `QFont::setFamilies` 设置完整中英文回退链（UI 8 字体、等宽 8 字体）+ `setPointSize` 物理单位（随 DPI 自适应）+ `setStyleHint(TypeWriter)` 确保等宽回退；`main.cpp` 改用 `uiFont(10)`；7 处 `QFont("Consolas")` 替换为 `monospaceFont(10)`；`DebugPanel` 字号缩减从 pixelSize 改 pointSize。全量 1773/1773 测试通过。
