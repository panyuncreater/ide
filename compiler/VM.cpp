...
const BytecodeChunk* VM::findMethodChunk(const std::string& className, const std::string& methodName) const {
    // P4 fix: 先查缓存
...
// ---- 数组内建方法 ----
VMResult VM::dispatchArrayBuiltin(const Value& obj, BuiltinMethod method, const std::string& methodName,
...
// ---- 字典内建方法 ----
VMResult VM::dispatchDictBuiltin(const Value& obj, BuiltinMethod method, const std::string& methodName,
                                 uint8_t argCount, uint16_t receiverVarIdx, uint8_t receiverLocalSlotByte, size_t& ip,
                                 OpCode op, int instrLen) {
    SmallArgs<Value> args(argCount);
    for (int i = argCount - 1; i >= 0; --i)
...
VMResult VM::dispatchStringBuiltin(const Value& obj, BuiltinMethod method, const std::string& methodName,
...
VMResult VM::executeArithOps(OpCode op, size_t& ip) {
    ...
    case OpCode::OP_NEGATE: {
        ...
        if (stack_.empty())
            return runtimeError("栈下溢：NEGATE 运算需要一个操作数");
        auto& top = stack_.back();
        if (top.isInt()) {
            if (OverflowCheck::negateOverflow(top.intVal()))
...
VMResult VM::executeCompareOps(OpCode op, size_t& ip) {
    ...
    case OpCode::OP_NOT: {
        if (stack_.empty())
            return runtimeError("栈下溢：NOT 运算需要一个操作数");
        // P17 fix: 原地修改栈顶
        ...
        return orderedCompare(
            [](const Value& l, const Value& r) {
                if (l.isString() && r.isString())
                    return l.stringVal() < r.stringVal();
